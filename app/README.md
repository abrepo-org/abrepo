...
* No restrictions for prod sending
...
Two tables: Tag and Taggings

* `Tag`: `ActsAsTaggable::Tag` - stores the tag name and count.
* `Tagging`: `ActsAsTaggable::Tagging` - "join" table that links  `Tag` to the `Taggable`

`Taggable` is the entity that is tagged. It's not db backed, but is
...
For queries `tag_list` is query-based and always loaded (not cached),
so it's best to avoid this method and do an in-memory
`map(&:tag).pluck(:name)` to avoid db.
